...
class EnhancedNinjaDependencyParser:
    def __init__(self, build_file_path, ninja_executable="ninja"):
        ...
        targets = (
            subprocess.run(
                [ninja_executable, "-t", "targets", "all"],
                cwd=self.build_dir,
                capture_output=True,
                text=True,
                timeout=30,
        ...
        [
            ...
            "HELP",
            "HIP_COMPILER",
            "HIP_EXECUTABLE_LINKER",
            "RERUN_CMAKE",
            "VERIFY_GLOBS",
            "phony",
        ]

        executables = [t.split(": ")[0] for t in targets if "_EXECUTABLE" in t]
        objects = [t.split(": ")[0] for t in targets if "_COMPILER" in t]
        import pdb
