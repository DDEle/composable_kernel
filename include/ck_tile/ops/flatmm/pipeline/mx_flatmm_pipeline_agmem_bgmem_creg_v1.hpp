...
struct MXF4FlatmmPipelineAGmemBGmemCRegV1 : FlatmmPipelineAGmemBGmemCRegV1<Problem, PipelinePolicy>
{
                                   ...
                                   void* p_smem_pong) const
    {
                    ...
                    static_for<0, NIterPerWarp / NXdlPack, 1>{}([&](auto nIter_pack) {
                        static_for<0, KXdlPack, 1>{}([&](auto ikxdl) {
                            static_for<0, MXdlPack, 1>{}([&](auto imxdl) {
                                ...
                                // barrier as ds_load A(2i) and buffer_load_lds A(2i + 1)
                                // finished
                                if constexpr(k_iter == KIterPerWarp - 1 &&
                                             m_iter == MIter_2nd_last &&
                                             nIter_pack == NIterPerWarp / NXdlPack - 1)
                                {
                                }
                            });
                    ...
                    static_for<0, NIterPerWarp / NXdlPack, 1>{}([&](auto nIter_pack) {
                        static_for<0, KXdlPack, 1>{}([&](auto ikxdl) {
                            static_for<0, MXdlPack, 1>{}([&](auto imxdl) {
                                ...
                                // barrier as ds_load A(2i + 1) and buffer_load_lds A(2i + 2)
                                // finished
        ...
        if constexpr(HasHotLoop)
        {
            ...
            {
                // if(get_thread_id() < 64)
                //     printf("MXF4FlatmmPipelineAGmemBGmemCRegV1 main_body_implx2\n");
                main_body_implx2();
                iCounter--;
            } while(iCounter > 0);
        }
