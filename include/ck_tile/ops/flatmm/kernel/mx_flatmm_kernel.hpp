                        ...
                        const SplitKBatchOffset& splitk_batch_offset)
    {
        const auto& a_tensor_view = [&]() {
            ...
            return make_naive_tensor_view<address_space_enum::global>(
                a_ptr,
                make_tuple(kargs.M, splitk_batch_offset.splitted_k / APackedSize),
                make_tuple(kargs.stride_A / APackedSize, 1),
                number<MXFlatmmPipeline::GetVectorSizeA()>{},
                number<1>{});
        }();

        constexpr index_t kKPerBlock    = MXFlatmmPipeline::kKPerBlock / BPackedSize;
        constexpr index_t kNWarpTile    = BlockGemmShape::WarpTile::at(I1);
        constexpr index_t flatKPerBlock = kKPerBlock * kNWarpTile;
        const index_t kFlatKBlocks      = kargs.K / BPackedSize / kKPerBlock;
