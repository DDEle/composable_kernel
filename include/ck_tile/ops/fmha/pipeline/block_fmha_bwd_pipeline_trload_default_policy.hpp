...
struct BlockFmhaBwdPipelineTrLoadDefaultPolicy
{
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeXDramTileDistribution()
    {
        ...
        constexpr index_t K2 = GetAlignmentK<Problem>();            // 8
        constexpr index_t K1 = WarpAlignmentBytes / sizeof(T) / K2; // 8
        ...
        constexpr index_t N1 = 16 / N2;
        constexpr index_t N0 = RowsPerBlock / N1 / N2;
        static_assert((N0 * N1 * N2 == RowsPerBlock) && (kWarps == 1) &&
                          (K1 * N2 == get_warp_size()),
                      "RowsPerBlock not divisible");

        return make_static_tile_distribution(
            tile_distribution_encoding<sequence<>,
                                       tuple<sequence<1, N0, N1, N2>, sequence<K0, K1, K2>>,
                                       tuple<sequence<1>, sequence<1, 2>>, // N2 K1
                                       tuple<sequence<0>, sequence<3, 1>>,
                                       sequence<1, 2, 1, 2>, //  N0 K0  N1 K2
                                       sequence<1, 0, 2, 2>>{});
    }
